...
export default function PublicSimulator() {
    ...
    useEffect(() => {
        ...
        if (tplParam) {
            ...
            if (tpl) { setSelectedTemplate(tpl); setActiveRules(tpl.rules); setRulesJson(JSON.stringify(tpl.rules, null, 2)); setTxInput(JSON.stringify(tpl.dataset.data, null, 2)); setHasInteracted(true); }
        }
    }, []);
    ...
    const txLabels: Record<number, string> = { 100: "100", 1000: "1K", 10000: "10K", 1000000: "Run Stress Test (1M Tx)" };
                ...
                <section className={styles.canvas} ref={resultsRef}>
                    {!result ? (
                        ...
                        <div className={styles.scenarioIntro}>
                            <h2 className={styles.introTitle}>{activeProfile.title}</h2>
                            <div className={styles.contextBox}>
                                <h4>{t.contextIntro.problem}</h4>
                                <p>{activeProfile.context.problemStatement}</p>
                            </div>
                            <div className={styles.contextBox}>
                                <h4>{t.contextIntro.solution}</h4>
                                <p>{activeProfile.context.tempusSolution}</p>
                            </div>
                            <div className={styles.ctaBox}>
                                <p className={styles.ctaText}>{t.contextIntro.cta}</p>
                            </div>
                        </div>
                        )
                    ) : (
